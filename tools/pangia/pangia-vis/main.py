...
p = figure(plot_height=config['displays']['dot_plot_height'], plot_width=config['displays']['dot_plot_width'], title="", y_axis_type="log",
...
data_table = DataTable(
    ...
    height=200)
result_table = widgetbox(data_table)
...
dl_btn.callback = CustomJS(args=dict(source=dp_source), code=open(join(dirname(__file__), "static/javascript/download.js")).read())
coff_btn.on_click( cutoff_toggle )

ctrl_panel_width = config['displays']['total_width']-config['displays']['dot_plot_width']-10
ctrl_panel = widgetbox(*controls, width=ctrl_panel_width)
