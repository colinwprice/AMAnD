...
def parse_params(ver):
	...
	p.add_argument('-db', '--database',
			metavar='[BWA_INDEX]', type=str, nargs='*',
					help="Name/path of BWA-MEM index(es). [default: None]")
	...
	default_val = 10000
	p.add_argument( '-xv','--maxVmr', metavar='<FLOAT>', type=float, default=default_val,
					help="Maximum variance-to-mean ratio (vmr) for the depth of mapped references [default: %s]"%default_val)
	...
	if not args_parsed.dbPath:
		...
		if args_parsed.database and "/" in args_parsed.database[0] and os.path.isfile( args_parsed.database[0] + ".ann" ):
...
parallel -j$THREAD --halt soon,fail=1 'printf "@SQ\\tSN:"%s"\\tLN:"%s"\\n" `printf {/.}` `printf {/.} | cut -d"|" -f2` > merged_sam/{};\
  cat sam_*/{} >> merged_sam/{};\
  ...
  samtools depth -d1000000 merged_sam/{/.}.sorted.bam > merged_sam/{/.}.sorted.depth' :::: sam_uniq.list

find merged_sam -type f -name '*.sam' | cut -d'|' -f3 | sort -u > taxid_uniq.list

parallel -j$THREAD --halt soon,fail=1 'printf {}"\\t"; depth_stats.py merged_sam/*\|{}\|[A-Z]*.sorted.depth' :::: taxid_uniq.list > tax_linear_cov_stats.txt
set +xe
"""
	...
	cmd = "set -o pipefail; set -x; PATH=%s:$PATH; bash %s/ref_linear_cov.sh %s %s"%(bindir, tempdir, tempdir, threads)
	...
	if argvs.debug:
		print_message( "[INFO] STDOUT: %s"%outs, argvs.silent, begin_t, logfile )

	if exitcode == 0:
		return '%s/tax_linear_cov_stats.txt'%tempdir
	else:
		print_message( "[INFO] STDERR: %s"%errs, argvs.silent, begin_t, logfile, True )
		return None
...
def processSAMfile( sam_fn, numthreads, numlines ):
	...
	lcov_stats_path = linear_cov_calculation( argvs.tempdir, argvs.threads )

	lc={}
	if lcov_stats_path:
		with open(lcov_stats_path) as c:
			for line in c:
				(tid, ll, rlen, mean, sd, cv, vmr) = line.rstrip().split('\t') #get reference name and linear length
				lc[tid]={}
				lc[tid]['LL']  = int(ll)
				lc[tid]['MRL'] = int(rlen)
				lc[tid]['MN']  = float(mean)
				lc[tid]['SD']  = float(sd)
				lc[tid]['CV']  = float(cv)
				lc[tid]['VMR']  = float(vmr)
		c.close()

	if len(lc)==0:
		print_message( "[WARNING] Program stopped. No depths of strains.\n", argvs.silent, begin_t, logfile, True )

	tol_num_mapped_reads = 0
	for k in result:
		tol_num_mapped_reads += result[k]["PR"]

	return result, tol_num_mapped_reads, lc
...
def taxonomyRollUp(r, lc, patho_meta, tol_num_mapped_reads, mb, mr, ml, mc, md):
	"""taxonomyRollUp
	...
	arg2:
	  lc <DICT>: depth stats of each strain
	      -> taxid <DICT>: taxonomy ID of strain
		      -> "LL"  <FLOAT>: linear mapped length
			  -> "MRL" <FLOAT>: total length of references
			  -> "MN"  <FLOAT>: mean of DOC
			  -> "SD"  <FLOAT>: SD of DOC
			  -> "CV"  <FLOAT>: coefficient of variation of DOC (SD/MN)
	...
	Returns:
	  res_rollup <DICT>: read mapping results roll up to superkingdom
		  -> taxid <DICT>: taxid as keys
			  ...
			  -> "LC"   <FLOAT>: best linear coverage (prop)
			  -> "SD"   <FLOAT>: SD of the best linear coverage strain
			  -> "CV"   <FLOAT>: the minimum CV of it's strains
	...
	# apply cutoffs at strain level and rollup to higher levels
	for taxid in res_rollup_str:
		...
		if taxid in lc:
			res_rollup_str[taxid]["LL"]  = lc[taxid]['LL']
			res_rollup_str[taxid]["MRL"] = lc[taxid]['MRL']
			res_rollup_str[taxid]["MN"]  = lc[taxid]['MN']
			res_rollup_str[taxid]["SD"]  = lc[taxid]['SD']
			res_rollup_str[taxid]["CV"]  = lc[taxid]['CV']
			res_rollup_str[taxid]["VMR"] = lc[taxid]['VMR']
		else:
			res_rollup_str[taxid]["LL"]  = 0
			res_rollup_str[taxid]["MRL"] = 0
			res_rollup_str[taxid]["MN"]  = 0
			res_rollup_str[taxid]["SD"]  = 0
			res_rollup_str[taxid]["CV"]  = 0
			res_rollup_str[taxid]["VMR"] = 0
		...
		for rank in lng:
			...
			if tid in res_rollup:
				...
				if res_rollup_str[taxid]["LC"] > res_rollup[tid]["LC"]:
					...
					res_rollup[tid]["SD"] = res_rollup_str[taxid]["SD"] 
				if res_rollup_str[taxid]["CV"] < res_rollup[tid]["CV"]:
					res_rollup[tid]["CV"] = res_rollup_str[taxid]["CV"]
				if res_rollup_str[taxid]["VMR"] < res_rollup[tid]["VMR"]:
					res_rollup[tid]["VMR"] = res_rollup_str[taxid]["VMR"]
			else:
...
def outputResultsAsReport(res_rollup, o, relAbu, user_taxid, display_fields, score_method, ms, mc, mb, mr, ml, md, mrd, xv, display_all):
	...
	for tid in res_rollup:
		...
		if rank in major_ranks:
			...
			if (str(score) != "NA" and ms > float(score) or
					...
					mrd > res_rollup[tid]["rsDCnr"] or
					xv < res_rollup[tid]["VMR"] ):
				abundance = 0
			else:
	...
	full_fields  = "\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (
						...
						"LINEAR_LENGTH", "TOTAL_BP_MAPPED", "RS_DEPTH_COV", "FLAG", "SD", 
						"CV", "VMR"
	...
	for rank in sorted( major_ranks, key=major_ranks.__getitem__ ):
		...
		for tid in sorted( taxas, key=taxas.__getitem__, reverse=True ):
			...
			note += "filtered out by maxVmr. "    if xv < res_rollup[tid]["VMR"] else ""
			...
			basic_fields = "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (
				...
				0 if note else "%.4f"%(output[rank]["RES"][tid]/output[rank]["TOT_ABU"]) # 13."REL_ABUNDANCE"                                                            
			)
			...
			full_fields = "\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (
				...
				res_rollup[tid]["SD"],          # 28."SD"
				res_rollup[tid]["CV"],          # 29."CV"
				res_rollup[tid]["VMR"],          # 30."VMR"
			) if "full" in display_fields else ""
	...
	for filename in in_sam_files:
		with open( filename ) as f:
			for line in f:
				...
				# skip read flagged "mate unmapped"
				if samflag & 8:
					continue
				...
				# if it's the last sam file, process and output directly
				if filename == in_sam_files[-1]:
					tol_mapped += 1
					...
					if final_sam == "HOST":
						# filter out HOST reads
						tol_host += 1				
						del sam[readname][mate]
						if not len(sam[readname]): del sam[readname]
...
def readMapping( reads, dbs, threads, add_options, seed_length, min_score, samfile, logfile ):
	"""
	...
	for db in dbs:
		...
		bwa_cmd    = "bwa mem -k%s -T%s %s -t%s %s %s" % (seed_length, min_score, add_options, threads, db, input_file )
		...
		cmd        = "%s %s 2>> %s | %s | %s | %s > %s" % ( bash_cmd, bwa_cmd, logfile, alnOnly_cmd, count_cmd, filter_cmd, sam_outpath )
...
def remove_unqualified_refs(refs, res_rollup, mb, mr, ml, mc, xv):
	qualified_refs=[]
	for ref in refs:
		(acc,leng,tid,tags) = ref.split('|')
		if mb > res_rollup[tid]["rsMRNb"] or mr > res_rollup[tid]["MR"] or ml > res_rollup[tid]["LL"] or mc > res_rollup[tid]["LC"] or xv < res_rollup[tid]["VMR"]:
			continue
		else:
...
if __name__ == '__main__':
	...
	print_message( "    Maximal VMR       : %s" % argvs.maxVmr,       argvs.silent, begin_t, logfile )
	...
	print_message( "    Aligner path      : %s" % dependency_check("bwa"), argvs.silent, begin_t, logfile )
	...
	if argvs.input:
		...
		(tol_host, tol_rootspec, tol_mapped, tol_reads) = readMapping( argvs.input, argvs.database, argvs.threads, argvs.addOptions, argvs.alignSeedLength, argvs.alignMinScore, samfile, logfile )
	...
	if argvs.mode == 'class':
		...
		(res, mapped_r_cnt, lc) = processSAMfile( os.path.abspath(samfile), argvs.threads, lines_per_process)
		...
		res_rollup = taxonomyRollUp(res, lc, patho_meta, mapped_r_cnt, argvs.minRsnb, argvs.minReads, argvs.minLen, argvs.minCov, argvs.minDc)
		print_message( "Done.", argvs.silent, begin_t, logfile )
		...
		if argvs.mode == 'report':
			outputResultsAsReport( res_rollup, out_fp, argvs.relAbu, argvs.taxonomy, argvs.reportFields, argvs.scoreMethod, argvs.minScore, argvs.minCov, argvs.minRsnb, argvs.minReads, argvs.minLen, argvs.minDc, argvs.minRsdcnr, argvs.maxVmr, argvs.displayAll )
